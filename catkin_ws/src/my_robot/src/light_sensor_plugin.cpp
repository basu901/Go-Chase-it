...
namespace gazebo
{
	...
	{
		//Make sure ROS node for Gazebo has been initialized
		if(!ros::isInitialized())
		{
			ROS_FATAL_STREAM(" Ros Node not initialized for Sensor Plugin,
unable to load 'lib_ros_api_plugin.so' in gazebo");
			return;
		}
		...
		const std::string &_format)
	{
		...
		if(!this->parentSensor->IsActive())
		{
			...
			{
				// do this first so there's a chance for sensor to run once after activated
		...
		else
		{
			if((*this->image_connect_count_)>0)
			{
				...
				if(cur_time - this->last_update_time_ >= this->update_period_)
				{
					...
					int startingPix = _width * ((int)(_height/2)-(int)(_fov/2))-(int)(_fov);
